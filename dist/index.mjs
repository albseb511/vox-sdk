...
var VoxProvider = ({
  ...
  const getAuthTokenAzure2 = React.useCallback(async () => {
    if (token && region && Date.now() - lastCallRef.current < 10 * 60 * 1e3) {
      return { token, region };
    }
    const response = await axios_default2.get("/token");
  ...
  const contextValue = { getAuthTokenAzure: getAuthTokenAzure2, token, region };
  return /* @__PURE__ */ React.createElement(AppContext.Provider, { value: contextValue }, children);
};
var useAppContext = () => {
...
var useToken = () => {
  ...
  const { getAuthTokenAzure: getAuthTokenAzure2, token, region } = useAppContext();
  useEffect(() => {
    const fetchToken = async () => {
      try {
        setLoading(true);
        await getAuthTokenAzure2();
      } catch (err) {
...
function useListen({
  ...
  const [userHasNotSpoken, setUserHasNotSpoken] = useState2(false);
  const [answers, setAnswers] = useState2([]);
  const [answer, setAnswer] = useState2("");
  const [loading, setLoading] = useState2(false);
  const [endOfSpeech, setEndOfSpeech] = useState2(false);
  const recognizerRef = useRef2(null);
  ...
  const startSpeechRecognition = () => new Promise(async (resolve, reject) => {
...
import * as sdk2 from "microsoft-cognitiveservices-speech-sdk";
import React2, { useEffect as useEffect3, useRef as useRef3, useState as useState3 } from "react";
...
function useTTSwithAI({
  ...
  const [isSpeaking, setIsSpeaking] = useState3(false);
  const [hasAllSentencesBeenSpoken, setHasAllSentencesBeenSpoken] = useState3(true);
  const [currentStreamedSentence, setCurrentStreamedSentence] = useState3("");
  const [streamedSentences, setStreamedSentences] = useState3([]);
  const [config, setConfig] = useState3(null);
  const didStreamCallbackGetCalled = useRef3(false);
  const playerRef = React2.useRef(null);
  const audioConfig = React2.useRef(null);
  const speechSythesizerRef = React2.useRef(null);
  const voice = "en-US-JennyNeural";
  const getAuthTokenAzureApi = useRef3(getAuthTokenAzure());
  const throttledCalledback = useThrottledCallback(
    (text) => startTextToSpeech(text),
    100
  );
  const isLocal = process.env.NODE_ENV === "development";
  const startTextToSpeech = async (text, cancelEndCallback) => {
    if (isSpeaking)
    ...
    const player = new sdk2.SpeakerAudioDestination();
    ...
    if (!config) {
      ...
      setConfig({ t: token, r: region });
      const speechConfig = sdk2.SpeechConfig.fromAuthorizationToken(
        token,
        region
      );
      speechConfig.speechSynthesisVoiceName = voice;
      speechSythesizerRef.current = new sdk2.SpeechSynthesizer(
        speechConfig,
        audioConfig.current
      );
    } else {
