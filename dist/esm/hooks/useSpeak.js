...
function useTTSwithAI({ shouldCallOnEnd = false, onEnd, }) {
    ...
    const [_currentStreamedSentence, setCurrentStreamedSentence] = useState("");
    const [streamedSentences, setStreamedSentences] = useState([]);
    const [config, setConfig] = useState(null);
    const didStreamCallbackGetCalled = useRef(false);
    const playerRef = React.useRef(null);
    const audioConfig = React.useRef(null);
    const speechSythesizerRef = React.useRef(null);
    ...
    const startTextToSpeech = (text, _cancelEndCallback) => __awaiter(this, void 0, void 0, function* () {
        var _a, _b, _c, _d, _e;
        if (isSpeaking)
